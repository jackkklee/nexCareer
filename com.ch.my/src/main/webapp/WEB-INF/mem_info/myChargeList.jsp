...
<body>
	...
	<div class="out">
		<div class="in">
			<c:if test="${startPage > pagePerBlock }">
				<a href="myChargeList.up?pageNum=${startPage-1 }">이전</a>
			</c:if>
			<c:forEach var="i" begin="${startPage}" end="${endPage}">
				<a href="myChargeList.up?pageNum=${i}">[${i}]</a>
			</c:forEach>
			<c:if test="${endPage < totPage }">
				<a href="myChargeList.up?pageNum=${endPage+1 }">다음</a>
			</c:if>
		</div>
	</div>
	...
	function a1(n) {
		$('article').load("myChargeList.up?pageNum="+n);
	}
	function a2(n) {
		$('article').load("myChargeList.up?pageNum="+n);
	}
