...
<article>
		...
		<div class="out">
			<div class="in">
				<c:if test="${startPage > pagePerBlock }">
					<a href="myDownList.up?pageNum=${startPage-1 }">이전</a>
				</c:if>
				<c:forEach var="i" begin="${startPage}" end="${endPage}">
					<a href="myDownList.up?pageNum=${i}">[${i}]</a>
				</c:forEach>
				<c:if test="${endPage < totPage }">
					<a href="myDownList.up?pageNum=${endPage+1 }">다음</a>
				</c:if>
			</div>
		</div>
	...
	function redown(file_num,id) {
		window.open("fileReSelect.up?file_num="+file_num+"&id="+id,"", "width=500 height=600");
	}
	</script>
</article>
</body>
