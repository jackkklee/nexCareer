...
#footer {
	width: 1000px;
	text-align:right; font-size: 8px; font-weight: lighter;	
...
$(document).ready(function() { 
	$.post('fileListSlide.up', 'category=' + $('#btn').val(),/* frm.category.value */
		function(data) {
			//$('#disp').html(data);
			$('#slide').html(data);
		});
	...
	function s1(ca, sub, key, n) {
		$('#disp').load("fileSearch.up?cate="+ca+"&subject="+sub+"&keyword="+key+"&pageNum="+n);
	}
	...
	$(function() {
		...
		$('#chargeForm').click(function() {
			$.post('chargeForm.up', function(data) {
				$('#disp').html(data);
			});
			return false;
		});
...
<body>
		...
		<div id="search">
			<form action="" name="frm" method="post" id="form">
				<select style="height: 22px" name="cate" id="cate">
					...
					placeholder="원하는 검색어를 입력하세요" required="required"> 
	...
	<div id="footer">
		(주) 판도라 웹하드 | 서울시 강남구 테헤란로 7길 7(역삼동 에스코빌딩 6층) / 대표자:홍길동 <br>
